...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts,
               norm, early_stop=False, boost=False, ad_alpha=False, func='ce', ds='cifar10'):
    # model.eval()
        ...
        for i in range(attack_iters):
            ...
            if func == 'ce':
                if not boost:
                    ...
                    loss = F.cross_entropy(output, y, reduction='none') + F.nll_loss(torch.log(1.001 - adv_probs + 1e-12), new_y)
            elif func == 'kl':
                # loss = F.kl_div(F.log_softmax(output, dim=1), F.softmax(model(normalize(X, ds=ds)), dim=1), reduction='sum')
                loss = F.kl_div(F.log_softmax(output, dim=1), F.softmax(model(normalize(X, ds=ds)), dim=1), reduction='batchmean')
            if func == 'ce':
                loss = loss.mean()
            loss.backward()
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for i, (data, label) in enumerate(train_loader):
            ...
            if args.wp and epoch > args.wp_warmup:
                ...
                wp = wp_adversary.calc_wp(inputs_adv=adv_data, targets=label, threshold=wp_threshold, weight=wp_weight, func='kl', inputs_clean=clean_data, beta=args.trades_beta)
                wp_adversary.perturb(wp)

            logits_adv = model(adv_data)
            loss_robust = F.kl_div(F.log_softmax(logits_adv, dim=1),
                               F.softmax(model(clean_data), dim=1),
                               reduction='batchmean')
            logits = model(clean_data)
            loss_natural = F.cross_entropy(logits, label)
            loss = loss_natural + args.trades_beta * loss_robust
            clean_acc = accuracy(logits.data, label)[0]
            robust_acc = accuracy(logits_adv.data, label)[0]

            train_acc_meter.update(clean_acc.item(), data.size(0))
            train_robust_loss_meter.update(loss.item(), data.size(0))
            ...
            if i % 100 == 0:
                log_info = 'step: {}/{}, train_acc: {:.4f}, train_robust_loss (TRADES): {:.6f}, train_robust_acc: {:.4f}'.format(
        ...
        log_info = 'epoch: {}, train_time: {:.1f}s, test_time: {:.1f}s, lr: {:.6f}, alpha: {:.6f}, epsilon: {:.6f}, attack_iters: {:.1f}, train_acc: {:.4f}, train_robust_loss (TRADES): {:.6f}, train_robust_acc: {:.4f}, test_acc: {:.4f}, test_{}_acc: {:.4f}'.format(
