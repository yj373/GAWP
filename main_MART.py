...
def get_args():
    ...
    parser.add_argument('--mart-beta', default=5.0, type=float)
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for i, (data, label) in enumerate(train_loader):
            ...
            if args.wp and epoch > args.wp_warmup:
                if args.gawp:
                    with torch.no_grad():
                        robust_loss_before_wp = mart_loss(model, clean_data, adv_data, label, beta=args.mart_beta)
                    wp_weight_1 = (1.0 - F.softmax(robust_loss_before_wp, dim=0)) * data.size(0) # small loss
                    kappa = kappa.cuda()
                ...
                wp = wp_adversary.calc_wp(inputs_adv=adv_data, targets=label, threshold=wp_threshold, weight=wp_weight, func='mart', inputs_clean=clean_data, beta=args.mart_beta)
                wp_adversary.perturb(wp)

            loss = mart_loss(model, data, adv_data, label, beta=args.mart_beta)
